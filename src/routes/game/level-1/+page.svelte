<script lang="ts">
	...
	import { getExplanation } from '$lib/game/ExplanationService';
	import type { ExerciseType } from '$lib/types/game';

	// Exercise state
	let exercise = $state(createIntervalExercise('interval', 4));
	let currentQuestion = $state<ReturnType<typeof exercise.generateQuestion> | null>(null);
	...
	function startExercise() {
		exercise = createIntervalExercise('interval', 4);
	...
	async function playInterval() {
		if (!currentQuestion || hasPlayed) return;
	...
	function handleAnswer(answer: string) {
		if (!currentQuestion || currentQuestion.type !== 'interval') return;
...
<svelte:head>
	<title>Lesson 1: Unison & Octave - Melora</title>
</svelte:head>
